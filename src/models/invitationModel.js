...
// Chỉ định ra những Fields mà chúng ta không muon cho phép cập nhật trong hàm update()
...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    // Biến đổi một số dữ liệu liên quan tới ObjectId chuần chỉnh
...
const update = async (invitationId, updateData) => {
  try {
    ...
    // Đối với những dữ liệu liÃ quan ObjectId, biến đồi ở đây
    ...
    const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOneAndUpdate(
      { _id: new ObjectId(invitationId) },
      { $set: updateData },
      { returnDocument: 'after' } // sẽ trả về kết quả mới sau khi cập nhat
    )
    return result
  } catch (error) { throw new Error(error) }
}
